...
function M.keymap_setup(note_path)
  ...
  keymap({'n', 'v'}, '<A-f>', ":lua require('telescope.builtin').live_grep()<CR>", default_opts)
  keymap({'n', 'v'}, '<S-A-f>', ":lua require('telescope.builtin').live_grep({cwd='" .. note_path ..  "}')", default_opts)
end
